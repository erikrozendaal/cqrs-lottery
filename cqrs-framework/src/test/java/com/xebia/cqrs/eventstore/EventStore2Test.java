...
public class EventStore2Test {
    ...
    private EventStore2<String> subject = new EventStore2<String>();
    
    @Test
    public void should_create_event_stream_with_initial_version_and_events() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T1, asList("foo", "bar")));
        
        FakeEventSink sink = new FakeEventSink(Integer.class, 0, T1, asList("foo", "bar"));
        subject.loadEventsFromStream(ID_1, sink);
    ...
    public void should_fail_to_create_stream_with_duplicate_id() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T1, asList("foo", "bar")));
        try {
            subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T2, asList("baz")));
    ...
    public void should_store_events_into_stream() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T1, asList("foo", "bar")));
        subject.storeEventsIntoStream(ID_1, 0, new FakeEventSource2(Integer.class, 1, T2, asList("baz")));
        
        FakeEventSink sink = new FakeEventSink(Integer.class, 1, T2, asList("foo", "bar", "baz"));
        subject.loadEventsFromStream(ID_1, sink);
    ...
    public void should_store_separate_event_logs_for_different_event_streams() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T1, asList("foo", "bar")));
        subject.createEventStream(ID_2, new FakeEventSource2(Integer.class, 0, T2, asList("baz")));
        
        FakeEventSink sink1 = new FakeEventSink(Integer.class, 0, T1, asList("foo", "bar"));
        subject.loadEventsFromStream(ID_1, sink1);
        FakeEventSink sink2 = new FakeEventSink(Integer.class, 0, T2, asList("baz"));
        subject.loadEventsFromStream(ID_2, sink2);
    ...
    public void should_fail_to_store_events_into_stream_when_versions_do_not_match() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 1, T1, asList("foo", "bar")));
        try {
            subject.storeEventsIntoStream(ID_1, 0, new FakeEventSource2(Integer.class, 1, T2, asList("baz")));
    ...
    public void should_fail_to_store_events_into_stream_when_new_version_is_before_previous_version() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 5, T1, asList("foo", "bar")));
        try {
            subject.storeEventsIntoStream(ID_1, 5, new FakeEventSource2(Integer.class, 4, T2, asList("baz")));
    ...
    public void should_fail_to_store_events_into_stream_when_new_timestamp_is_before_previous_timestamp() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T2, asList("foo", "bar")));
        try {
            subject.storeEventsIntoStream(ID_1, 0, new FakeEventSource2(Integer.class, 1, T1, asList("baz")));
            fail("IllegalArgumentException expected");
        } catch (IllegalArgumentException expected) {
        }
    }
    ...
    public void should_fail_to_load_events_from_non_existing_event_stream() {
        try {
            subject.loadEventsFromStream(ID_1, new FakeEventSink(Integer.class, 0, T1, asList("foo")));
    ...
    public void should_fail_to_store_events_into_non_existing_event_stream() {
        try {
            subject.storeEventsIntoStream(ID_1, 0, new FakeEventSource2(Integer.class, 0, T1, asList("foo")));
            fail("EmptyResultDataAccessException expected");
        } catch (EmptyResultDataAccessException expected) {
        }
    }
    ...
    public void should_fail_to_load_events_from_stream_when_requested_version_is_before_first_event_version() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 2, T1, asList("foo", "bar")));
        try {
            subject.loadEventsFromStreamAtVersion(ID_1, 1, new FakeEventSink(Integer.class, 0, T2, asList("foo", "bar")));
            fail("EmptyResultDataAccessException expected");
        } catch (EmptyResultDataAccessException expected) {
        }
    }
    ...
    public void should_fail_to_load_events_from_stream_when_request_timestamp_is_before_first_event_timestamp() {
        subject.createEventStream(ID_1, new FakeEventSource2(Integer.class, 0, T2, asList("foo", "bar")));
        try {
            subject.loadEventsFromStreamAtTimestamp(ID_1, T1, new FakeEventSink(Integer.class, 0, T1, asList("foo", "bar")));
            fail("EmptyResultDataAccessException expected");
        } catch (EmptyResultDataAccessException expected) {
        }
    }
    
    private static class FakeEventSource2 implements EventSource2<String> {
        ...
        public FakeEventSource2(Class<?> type, long version, long timestamp, List<String> events) {
            this.type = type;
    ...
    private static final class FakeEventSink implements EventSink<String> {
        ...
        public FakeEventSink(Class<?> expectedType, long expectedVersion, long expectedTimestamp, List<String> expectedEvents) {
        ...
        public void setType(Class<?> actualType) {
