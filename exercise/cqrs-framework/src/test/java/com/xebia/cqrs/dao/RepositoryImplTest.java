...
public class RepositoryImplTest {
    ...
    public void setUp() {
        ...
        aggregateRoot.setVersion(2);
        aggregateRoot.loadFromHistory(asList(new GreetingEvent("test-id", 0, "Hi Erik"), new GreetingEvent("test-id", 1, "Hi Sjors")));
    ...
    public void shouldFailOnNonExistingAggregateRoot() {
        expect(eventStore.loadEventSource(FakeAggregateRoot.class, "test-id", 1)).andReturn(null);
        replay(eventStore);

        try {
            subject.get(FakeAggregateRoot.class, "test-id", 1);
            ...
            assertEquals("test-id", expected.getAggregateRootId());
        }
    }
    
    @Test
    public void shouldLoadAggregateRootFromEventStore() {
        expect(eventStore.loadEventSource(FakeAggregateRoot.class, "test-id", 1)).andReturn(aggregateRoot);
        replay(eventStore);
        
        FakeAggregateRoot result = subject.get(FakeAggregateRoot.class, "test-id", 1);
